...
function update() {
    ...
    if (amountBooster > 0) {
        ...
        if (iterator % 5 === 0) {
            amountBooster = amountBooster - (amountNyanCats + +amountSuperNyanCats + amountUltraNyanCat);
            if (amountBooster < 0) {
                amountBooster = 0;
            }
        }
    ...
    document.getElementById('buyCatNip').dataset.price = priceBooster;
    updatePoints();
    updateCanPurchase();
...
function updateFields() {
    ...
    // catnip
    if (amountBooster) {
        document.getElementById('amountCatNip').innerHTML = 'Catnip left: ' + amountBooster;
    } else {
